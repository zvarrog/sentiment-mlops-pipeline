...
def _task_train_logreg(**context):
    return _train_model_parallel("logreg", **context)


def _task_train_rf(**context):
    return _train_model_parallel("rf", **context)


def _task_train_gb(**context):
    return _train_model_parallel("hist_gb", **context)


def _task_select_best(**context):
    """Выбор лучшей модели из обученных в parallel режиме."""
    _setup_env(**context)
    import shutil
    from pathlib import Path
...
with DAG(
    ...
    def _branch_by_mode(**context):
        ...
        mode = _get_value(context, "execution_mode", "standard")
        if mode == "parallel":
            return ["train_logreg", "train_rf", "train_gb"]
    ...
    branch >> [train_logreg, train_rf, train_gb] >> select_best
